...
function isTriangle(a, b, c) {
  // Проверяем условие для всех трех пар сторон
...
function doRectanglesOverlap(rect1, rect2) {
  // Проверяем условия для пересечения
...
function getCommonDirectoryPath(pathes) {
  ...
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < pathPartsArray[0].length; i++) {
  ...
  return commonPathParts.length > 0 ? `${commonPathParts.join('/')}/` : '';
}
...
function evaluateTicTacToePosition(position) {
  ...
  // Если никто не выиграл и доска заполнена
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < 3; i++) {
    // eslint-disable-next-line no-plusplus
    for (let j = 0; j < 3; j++) {
  ...
  // Если никто не выиграл и доска полностью заполнена, это ничья
  return undefined;
}
